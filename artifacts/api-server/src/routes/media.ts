...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      req.log.error({ err: String(err), key }, "Server-side upload to R2 failed");
...
// Deprecated — was a Supabase presigned URL generator.  Returns 410 so any
// stale client surfaces the issue immediately instead of failing silently.

router.post("/media/upload-url", requireAuth, (_req, res) => {
  res.status(410).json({
    error: "GONE",
    message: "Direct presigned uploads are no longer supported. Use POST /api/media/upload instead.",
  });
});
