 ...
 *
 * The backend checks profiles.is_premium for the authenticated user.
 * If true, the subscription is confirmed active (common reinstall scenario).
 * If false, the user needs to tap "Subscribe" — Google Play will not charge
 * them again if they already have an active subscription on their account.
 */
export async function restoreSubscription(userId: string): Promise<SubscriptionResult> {
  console.log("[Billing] restoreSubscription — userId:", userId || "(empty)");
  ...
  const authToken = await getToken();
  if (!authToken) {
    console.error("[Billing] ABORT: missing auth token");
    return { success: false, error: "no_auth_token" };
  }
  ...
  try {
    ...
    try { json = await response.json() as typeof json; } catch { /* non-JSON */ }

    console.log("[Billing] restoreSubscription: HTTP", response.status, "success:", json.success, "restored:", json.restored, "error:", json.error ?? "(none)");

    if (!response.ok) {
      console.error("[Billing] restoreSubscription: backend error — HTTP", response.status);
